...
import { Loader2, Play, XCircle, EyeOff, Eye, RotateCw } from "lucide-react";
        ...
        <div className="flex h-full min-h-0 flex-col">
          <div className="flex-1 min-h-0 overflow-y-auto">
            ...
            <div className="grid gap-4 p-5">
              ...
              {selectedReport.status === "pending" && selectedVideo && (
                <label className="flex items-center gap-2.5 cursor-pointer py-1.5 ml-0.5">
                  <input
                    type="checkbox"
                    checked={banUser}
                    onChange={(e) => setBanUser(e.target.checked)}
                    disabled={saving}
                    className="h-4 w-4 rounded border-[#d6d3d1] text-red-600 focus:ring-red-500 cursor-pointer"
                  />
                  <span className="text-sm font-semibold text-[#292524]">
                    Khóa tài khoản của người đăng video
                  </span>
                </label>
              )}
          ...
          <div className="mt-auto flex shrink-0 flex-wrap justify-end gap-2 border-t border-[#e7e5e4] p-4 bg-white">
            {selectedReport.status === "pending" && (
              <>
                ...
                <button
                  type="button"
                  disabled={saving}
                  onClick={() => onUpdateStatus("action_taken", note, { ban_user: banUser })}
            ...
            {selectedReport.status === "dismissed" && (
              <button
                ...
                className="inline-flex h-10 cursor-pointer items-center gap-2 rounded-full bg-[#292524] px-5 text-[15px] font-medium text-white transition hover:bg-black disabled:cursor-not-allowed disabled:opacity-60 shadow-sm"
              >
                {saving ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : <RotateCw className="h-4 w-4" />}
                <span>Xử lý lại</span>
              </button>
            )}
