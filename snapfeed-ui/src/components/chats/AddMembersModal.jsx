...
export default function AddMembersModal({ open, onClose, meId, conversationId, existingParticipantIds = [], onMembersAdded }) {
  ...
  const loadDefaultUsers = useCallback(async () => {
    ...
    try {
      ...
      const filtered = results.filter(
        (u) => !existingParticipantIds.includes(Number(u.id))
      );
      setUsers(filtered);
    } catch (e) {
  ...
  const searchUsers = useCallback(async (query) => {
    ...
    try {
      ...
      const results = Array.isArray(data?.results) ? data.results : [];
      // Filter out self and existing participants
      const filtered = results.filter(
        (u) => Number(u.id) !== Number(meId) && !existingParticipantIds.includes(Number(u.id))
      );
      setUsers(filtered);
    } catch (e) {
  ...
  const handleAddMembers = async (e) => {
    ...
    try {
      ...
      const updatedConversation = await conversationsApi.addMembers(conversationId, userIds);
      ...
      console.error(err);
      alert("Không thể thêm thành viên: " + (err?.message || "Lỗi hệ thống"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    ...
    >
      <div
        className="relative flex h-[580px] max-h-[80vh] w-full max-w-md flex-col rounded-2xl bg-white shadow-xl dark:bg-zinc-900 border border-zinc-200 dark:border-white/10 animate-in zoom-in-95 duration-200"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-zinc-150 p-4 dark:border-white/10">
          <h2 className="text-base font-bold text-zinc-900 dark:text-white">Thêm thành viên vào nhóm</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        {/* Form Body */}
        <form onSubmit={handleAddMembers} className="flex min-h-0 flex-1 flex-col p-4 gap-4">
          {/* Search Member input */}
          <div className="flex flex-col gap-1.5">
            <label className="text-xs font-bold text-zinc-500 dark:text-zinc-400">Tìm kiếm bạn bè</label>
          ...
          <div className="flex-1 min-h-0 overflow-y-auto border border-zinc-150 dark:border-white/10 rounded-xl">
            {loading ? (
              ...
              <div className="flex h-full flex-col items-center justify-center py-10 px-4 text-center text-zinc-500 dark:text-white/55">
                ...
                <p className="text-xs font-semibold">Không có gợi ý hoặc người dùng mới nào</p>
                <p className="text-[11px] text-zinc-400 mt-0.5">
                  Tất cả bạn bè phù hợp có thể đã ở trong nhóm này rồi.
                </p>
              </div>
            ) : (
          ...
          <div className="flex justify-end gap-2 border-t border-zinc-100 pt-3 dark:border-white/5">
            ...
            >
              {submitting ? (
                ...
                `Thêm vào nhóm (${selectedUsers.length})`
              )}
            </button>
          </div>
        </form>
